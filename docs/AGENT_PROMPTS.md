...
## 4b. Medicines & conditions research

**Task:** Expand offline common/rare medicine and condition catalogs on My ID (search + browse).

> Research common and rare medicines and conditions. Grow `COMMON_MEDS` / `RARE_MEDS` and `COMMON_CONDITIONS` / `RARE_CONDITIONS` in `index.html` (ClinCalc-oriented common meds; orphan/specialty rare; NORD/MedicAlert-style rare conditions). Keep `catalogMinus` + curated quick seeds; recompute CSP; sync www mirror. Plan note: [`docs/plans/meds-conditions-research.md`](plans/meds-conditions-research.md).

**Status:** Applied onto current `main` via `cursor/wireless-charging-guidance-a8f0` / PR #3.
...
## 6. RedMed fr — French i18n

**Stay in this repo / workspace.** Do not create a `RedMed-fr` fork or second GitHub repo. French ships beside EN/ES in `index.html` `STRINGS`.

**Task:**

1. Add `STRINGS.fr` with full key parity to `STRINGS.en`.
2. Bottom-bar language control cycles **EN · ES · FR** (`LANG_ORDER`); detect `navigator.language` starting with `fr`.
