...
run_local_mac_setup() {
  local repo_url="${REDMED_GIT_URL:-https://github.com/RedmMed/RedMed.git}"
...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  echo "  git clone https://github.com/RedmMed/RedMed.git ~/RedMed" >&2
...
if [ -z "$ORIGIN" ]; then
  git remote add origin https://github.com/RedmMed/RedMed.git 2>/dev/null || true
  ORIGIN="$(git remote get-url origin 2>/dev/null || echo https://github.com/RedmMed/RedMed.git)"
fi
